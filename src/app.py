...
load_dotenv() # when you connect to a database, we need to provide parameters. .env : all the parameters are saved there, and then loaded before starting to work with the database 
print("Dotenv file was loaded")
# 1) Connect (your python program) to the database here using the SQLAlchemy's create_engine function
# to run the code in the terminal: python src/app.py
def connect():
...
# eliminated the con.execute of the tables otherwise it was going to make problems executing itself more times in the terminal
...
# print(insert_data)
# con.execute(insert_data) # executing after


# 4) Use pandas to print one of the tables as dataframes using read_sql function
data_frame = pd.read_sql("SELECT * FROM authors;", con) # could be called cursor
print(data_frame)